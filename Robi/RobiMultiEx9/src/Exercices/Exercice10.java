		...
		while(true) {
			...
			if(robiByName.containsKey(fin[0])) {
				...
				if(robiByName.get(fin[0]).containsMacro(fin[1])) {
					...
					/*
					* commande de demande de trace séparée du run précédent
					*  pour éviter le nullpointer exception dû à l'absence de fin[2]
					*/
					else {
						c.run(robiByName.get(fin[0]), w,new String[] {fin[1]});
						w.repaint();
					}
				}
			}
			...
			if(fin[0].equals("delete")) {
				...
				if (robiByName.containsKey(fin[1])) {
					...
					//Suppression des submorphs du robi � supprimer
